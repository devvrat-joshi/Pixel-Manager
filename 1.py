...
def main(stdscr):
    ...
    stdscr.addstr(
        0, w // 2 - len(path) // 2, path, curses.color_pair(5) + curses.A_BOLD
    )
    terminal = 0
    k = 0
    onboard = ""
    ...
    while 1:
        ...
        if key == 99 and not terminal:
            ...
            if len(k) > 3 * w // 5 - 30:
                stdscr.addstr(
                    h - 1, len(copy), k[: 3 * w // 5 - 30], curses.color_pair(7)
                )
            else:
        ...
        if (
            ...
            if onboard == "cd ..":
                ...
                if cur_row <= l:
                    ...
                    stdscr.addstr(
                        h - 2, w // 2 - len(menu[cur_row - 1]) // 2, menu[cur_row - 1]
                    )
                if terminal == 1:
                    stdscr.addstr(
                        0,
                        0,
                        "Terminal: " + path + " " * (w - len(path) - 10),
                        curses.color_pair(6),
                    )
                    ...
                    stdscr.addstr(0, 0, " " * w, curses.color_pair(5))
                    stdscr.addstr(
                        0, w // 2 - len(path) // 2, path, curses.color_pair(5)
                    )
                onboard = ""
            else:
                stdscr.addch(key)
        ...
        if key == curses.KEY_BTAB:
            if terminal == 0:
                pp = "Terminal: " + path
                stdscr.addstr(
                    0, 0, pp + " " * (w - len(path) - 10), curses.color_pair(6)
                )
                terminal = 1
                k = 0
                curses.cbreak()
            ...
            if cur_row >= l or cur_row > h - 4:
                if listings[cur_row]:
                    ...
                    empty_right(stdscr)
                    stdscr.addstr(
                        h // 2, w // 2 - len(file) // 2, file, curses.color_pair(3)
                    )
                cur_row += 1
            ...
            key == curses.KEY_ENTER or key == 10 or key == 13 or key == curses.KEY_RIGHT
        ):
            try:
