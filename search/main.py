...
def main(stdscr):
    ...
    while 1:
        ...
        if key == ord("p"):
            cur_row = start_editor(stdscr,menu[cur_row-1],cur_row)
            print_menu(stdscr, listings, cur_row-1, "", menu)
            for i in range(1,h-2):
                stdscr.addstr(i,w-wn," "*wn,curses.color_pair(11))
            curses.init_pair(3, 3, 55)
        ...
        if key == ord("e"):
            # editor
            cur_row = editor.start_editor(stdscr,menu[cur_row-1],cur_row)
            print_menu(stdscr, listings, cur_row-1, menu[cur_row-1], menu)
            for i in range(1,h-2):
                stdscr.addstr(i,w-wn," "*wn,curses.color_pair(11))
            curses.init_pair(3, 3, 55)
        ...
        if key==ord("d"):
            ...
            key = stdscr.getch()
            if(key==ord("r")):
                if(os.path.isdir(menu[cur_row-1])):
                    shutil.rmtree(menu[cur_row-1])
                else:
                    os.remove(menu[cur_row-1])
                cur_row = 1
                menu = os.listdir()
                menu.sort()
                listings = []
